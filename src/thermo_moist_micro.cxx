...
ThermoMoistMicro::ThermoMoistMicro(Model *modelin, Input *inputin) : Thermo(modelin, inputin)
{
  swthermo = "moist_micro";
...
ThermoMoistMicro::~ThermoMoistMicro()
{
  delete[] thl0;
  delete[] qt0;
  delete[] thvref;
...
void ThermoMoistMicro::create(Input *inputin)
{
...
void ThermoMoistMicro::getMask(Field3d *mfield, Field3d *mfieldh, Mask *m)
{
  if(m->name == "ql")
  {
...
void ThermoMoistMicro::calcMask_ql(double * restrict mask, double * restrict maskh, double * restrict maskbot,
...
void ThermoMoistMicro::calcMask_qlcore(double * restrict mask, double * restrict maskh, double * restrict maskbot,
                                  int * restrict nmask, int * restrict nmaskh, int * restrict nmaskbot,
...
void ThermoMoistMicro::execStats(Mask *m)
{
  ...
  if(grid->swspatialorder == "2")
  {
    ...
    {
      stats->calcDiff_2nd(fields->atmp["tmp1"]->data, m->profs["bdiff"].data, grid->dzhi, fields->sp["thl"]->visc, sloc,
                          fields->atmp["tmp4"]->data, stats->nmaskh);
    }
  }
  else if(grid->swspatialorder == "4")
  {
    ...
    stats->calcDiff_4th(fields->atmp["tmp1"]->data, m->profs["bdiff"].data, grid->dzhi4, fields->sp["thl"]->visc, sloc,
                        fields->atmp["tmp4"]->data, stats->nmaskh);
  }
...
void ThermoMoistMicro::execCross()
{
  int nerror = 0;
...
void ThermoMoistMicro::execDump()
{
...
bool ThermoMoistMicro::checkThermoField(std::string name)
{
  if(name == "b" || name == "ql")
    return false;
  else
...
#ifndef USECUDA
void ThermoMoistMicro::getThermoField(Field3d *fld, Field3d *tmp, std::string name)
{
  const int kcells = grid->kcells;
...
#ifndef USECUDA
void ThermoMoistMicro::getBuoyancySurf(Field3d *bfield)
{
...
#ifndef USECUDA
void ThermoMoistMicro::getBuoyancyFluxbot(Field3d *bfield)
{
  calcBuoyancyFluxBot(bfield->datafluxbot, fields->sp[thvar]->databot, fields->sp[thvar]->datafluxbot, fields->sp["qt"]->databot, fields->sp["qt"]->datafluxbot, thvrefh);
}
#endif

void ThermoMoistMicro::getProgVars(std::vector<std::string> *list)
{
 ...
 */
void  ThermoMoistMicro::calcBaseState(double * restrict pref,     double * restrict prefh,
...
void ThermoMoistMicro::calcBuoyancyTend_2nd(double * restrict wt, double * restrict thl, double * restrict qt, 
...
void ThermoMoistMicro::calcBuoyancyTend_4th(double * restrict wt, double * restrict thl,  double * restrict qt, 
                                       double * restrict ph, double * restrict thlh, double * restrict qth, double * restrict ql,
                                       double * restrict thvrefh)
{
  int ijk,jj,ij;
...
void ThermoMoistMicro::calcBuoyancy(double * restrict b, double * restrict thl, double * restrict qt, double * restrict p, double * restrict ql,
...
void ThermoMoistMicro::calcLiquidWater(double * restrict ql, double * restrict thl, double * restrict qt, double * restrict p)
{
  int ijk,jj,kk;
  double ex;
...
void ThermoMoistMicro::calcN2(double * restrict N2, double * restrict thl, double * restrict dzi, double * restrict thvref)
{
  int ijk,jj,kk;
  jj = grid->icells;
  kk = grid->ijcells;
...
void ThermoMoistMicro::calcBuoyancyBot(double * restrict b,   double * restrict bbot,
...
void ThermoMoistMicro::calcBuoyancyFluxBot(double * restrict bfluxbot, double * restrict thlbot, double * restrict thlfluxbot, double * restrict qtbot, double * restrict qtfluxbot,
                                      double * restrict thvrefh)
{
  int ij,jj,kstart;
  jj = grid->icells;
...
void ThermoMoistMicro::initStat()
{
...
void ThermoMoistMicro::initCross()
{
...
void ThermoMoistMicro::initDump()
{
...
// INLINE FUNCTIONS
inline double ThermoMoistMicro::buoyancy(const double exn, const double thl, const double qt, const double ql, const double thvref)
{
  return grav * ((thl + Lv*ql/(cp*exn)) * (1. - (1. - Rv/Rd)*qt - Rv/Rd*ql) - thvref) / thvref;
}

inline double ThermoMoistMicro::buoyancyNoql(const double thl, const double qt, const double thvref)
{
  return grav * (thl * (1. - (1. - Rv/Rd)*qt) - thvref) / thvref;
}

inline double ThermoMoistMicro::buoyancyFluxNoql(const double thl, const double thlflux, const double qt, const double qtflux, const double thvref)
{
  return grav/thvref * (thlflux * (1. - (1.-Rv/Rd)*qt) - (1.-Rv/Rd)*thl*qtflux);
}

inline double ThermoMoistMicro::satAdjust(const double thl, const double qt, const double p, const double exn)
{
...
inline double ThermoMoistMicro::exner(const double p)
{
  return pow((p/p0),(Rd/cp));
}

inline double ThermoMoistMicro::exn2(const double p)
{
  double dp=p-p0;
  return (1+(dp*(ex1+dp*(ex2+dp*(ex3+dp*(ex4+dp*(ex5+dp*(ex6+ex7*dp)))))))); 
}

inline double ThermoMoistMicro::qsat(const double p, const double T)
{
  return ep*esat(T)/(p-(1-ep)*esat(T));
}

inline double ThermoMoistMicro::esat(const double T)
{
